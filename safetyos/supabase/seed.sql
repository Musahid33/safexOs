...
insert into near_misses
  (company_id, report_number, nm_date, nm_time, location_id, department_id, employee_id, description, category, severity, photos, root_cause, corrective_action, preventive_action, capa_id, status, assigned_to, officer_remarks, timeline, search_text)
values
  ('f0000000-0000-4000-8000-000000000001', 'NM-2026-0001', '2026-07-28', '09:40', (select id from locations where name='Warehouse A' limit 1), (select id from departments where name='Warehouse' limit 1), (select id from employees where employee_code='EMV-005'), 'Pallet stacked on mezzanine edge almost toppled onto the walkway below. Area barricaded immediately.', 'Falling Object', 'High', '[]', 'Inadequate stacking height and no edge protection on mezzanine.', 'Pallet relocated; edge mesh guard installed.', 'Weekly stacking audit added to warehouse checklist.', null, 'Open', '10000000-0000-4000-8000-000000000003', '', '[{"date":"2026-07-28","event":"Reported","note":"Reported via app","actor":"Arun Prasad"}]', 'NM-2026-0001 Falling Object Warehouse A High'),
  ('f0000000-0000-4000-8000-000000000001', 'NM-2026-0002', '2026-07-15', '14:20', (select id from locations where name='Rolling Mill' limit 1), (select id from departments where name='Production' limit 1), (select id from employees where employee_code='EMV-003'), 'Hydraulic hose leaked near walkway creating slip hazard during shift change.', 'Chemical Exposure', 'Medium', '[]', 'Hose past replacement interval.', 'Hose replaced; spill cleaned.', 'PM schedule updated for hose replacement intervals.', null, 'Under Review', '10000000-0000-4000-8000-000000000003', 'Investigation in progress.', '[{"date":"2026-07-15","event":"Reported","note":"Reported via app","actor":"Vikram Singh"}]', 'NM-2026-0002 Chemical Exposure Rolling Mill Medium'),
  ('f0000000-0000-4000-8000-000000000001', 'NM-2026-0003', '2026-06-30', '11:05', (select id from locations where name='Assembly Line 2' limit 1), (select id from departments where name='Production' limit 1), (select id from employees where employee_code='EMV-001'), 'Forklift took a sharp turn close to pedestrian route; operator braked in time.', 'Vehicle Movement', 'Low', '[]', 'Shared traffic route without physical segregation.', 'Pedestrian walkway marked with barriers.', 'Traffic management plan under revision.', null, 'Verified', '10000000-0000-4000-8000-000000000003', 'Controls verified effective.', '[{"date":"2026-06-30","event":"Reported","note":"Reported via app","actor":"Ramesh Kumar"},{"date":"2026-07-05","event":"Investigated","note":"RCA done","actor":"Karthik Selvam"},{"date":"2026-07-18","event":"Verified","note":"Controls in place","actor":"Karthik Selvam"}]', 'NM-2026-0003 Vehicle Movement Assembly Line 2 Low'),
  ('f0000000-0000-4000-8000-000000000001', 'NM-2026-0004', '2026-06-12', '16:45', (select id from locations where name='Electrical Substation' limit 1), (select id from departments where name='Maintenance' limit 1), (select id from employees where employee_code='EMV-002'), 'Loose cable gland observed on motor terminal box before energisation.', 'Electrical', 'High', '[]', 'Incomplete pre-energisation check.', 'Gland tightened and torque-marked.', 'LOTO + pre-energisation checklist rollout.', null, 'CAPA Pending', '10000000-0000-4000-8000-000000000003', 'CAPA raised with maintenance head.', '[{"date":"2026-06-12","event":"Reported","note":"Reported via app","actor":"Priya Sharma"}]', 'NM-2026-0004 Electrical Electrical Substation High'),
  ('f0000000-0000-4000-8000-000000000001', 'NM-2026-0005', '2026-05-22', '10:10', (select id from locations where name='Paint Shop' limit 1), (select id from departments where name='Production' limit 1), (select id from employees where employee_code='EMV-001'), 'Solvent drum left unsealed near ignition source; shift supervisor intervened.', 'Fire / Smoke', 'Critical', '[]', 'Inadequate chemical handling discipline.', 'Drums resealed; storage plan revised.', 'Chemical handling refresher training.', null, 'Closed', '10000000-0000-4000-8000-000000000003', 'Closed after effectiveness check.', '[{"date":"2026-05-22","event":"Reported","note":"Reported via app","actor":"Ramesh Kumar"},{"date":"2026-05-28","event":"Investigated","note":"RCA done","actor":"Karthik Selvam"},{"date":"2026-06-10","event":"CAPA Closed","note":"Actions verified","actor":"Karthik Selvam"}]', 'NM-2026-0005 Fire Smoke Paint Shop Critical'),
  ('f0000000-0000-4000-8000-000000000001', 'NM-2026-0006', '2026-08-06', '08:55', (select id from locations where name='Loading Bay' limit 1), (select id from departments where name='Logistics' limit 1), (select id from employees where employee_code='EMV-007'), 'Sling strap with visible cut marks was picked up for lifting before inspection.', 'Equipment Failure', 'Medium', '[]', 'Lifting gear not inspected before use.', 'Sling quarantined and destroyed.', 'Colour-coded monthly inspection tags for all slings.', null, 'Open', '10000000-0000-4000-8000-000000000003', '', '[{"date":"2026-08-06","event":"Reported","note":"Reported via app","actor":"Imran Khan"}]', 'NM-2026-0006 Equipment Failure Loading Bay Medium');
